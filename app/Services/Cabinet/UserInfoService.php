...
class UserInfoService
{
    public function userFillBalancee($userData)
    {
        try {
            ...
            $userTransaction->service_id = Auth::id();
            $userTransaction->action_user_id = Auth::id();
    ...
    public function infoChangePassport($userData)
    {
        try {
            $passport = Passport::where('passport_inn', $userData['passport_inn'])->orwhere('passport_number', $userData['passport_number'])->first();
            if ($passport && $passport->status) {

                if ($passport->status) {
                    $lang['ru'] = 'Паспорт  Пользователя имеется';
                    $lang['uz'] = 'Foydalanuvchi pasporti mavjud';
                    return response()->json($lang, Response::HTTP_FORBIDDEN);
                } else {
                    $passport->passport_expired = $userData['passport_expired'];
                    $passport->passport_given = $userData['passport_given'];
                    $passport->passport_inn = $userData['passport_inn'];
                    $passport->passport_number = $userData['passport_number'];
                    $passport->passport_inn = $userData['passport_inn'];
                    if (isset($userData['passport_photo'])) {
                        if ($passport->passport_photo && file_exists(public_path('/files/passport/' . $passport->passport_photo))) {
                            unlink(public_path('/files/passport/' . $passport->passport_photo));
                        }
                        $imageName = (string) Str::uuid() . '-' . Str::random(15) . '.' . $userData['passport_photo']->getClientOriginalExtension();
                        $userData['passport_photo']->move(public_path('/files/passport'), $imageName);
                        $passport->passport_photo = $imageName;
                    }
                    $passport->save();
                    return response()->json($passport, Response::HTTP_OK);
                }
            } else {
